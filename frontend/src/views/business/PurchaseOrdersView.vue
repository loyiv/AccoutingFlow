<template>
  <div class="page">
    <div class="top">
      <h2>采购订单</h2>
      <div class="actions">
        <button class="btn" @click="openCreate">新建采购订单</button>
        <button class="btn" @click="openExample">一键填充示例</button>
        <button class="btn" @click="load">刷新</button>
      </div>
    </div>

    <ExampleCard title="操作示例（企业日常）：采购办公用品" subtitle="目标：生成采购草稿 → 审批 → 过账，形成应付。">
      <ol class="ol">
        <li>点击“<b>一键填充示例</b>”，检查供应商=示例供应商，科目=5001 管理费用。</li>
        <li>点击“保存并生成草稿”，进入草稿页后依次点击：审批 → 过账。</li>
        <li>回到本页可看到单据与草稿状态变化；到“待过账”也能看到对应草稿。</li>
      </ol>
    </ExampleCard>

    <table class="table">
      <thead>
        <tr><th>单号</th><th>日期</th><th>供应商</th><th>状态</th><th>总额</th><th>草稿</th></tr>
      </thead>
      <tbody>
        ...
        <tr v-if="!docs.length"><td colspan="6" class="muted">暂无采购订单</td></tr>
      </tbody>
    </table>
    ...
    <div v-if="showDialog" class="modal">
      <div class="modal-card">
        ...
        <div class="form">
          ...
          <h4>明细行（借：费用/存货；贷：应付账款，默认税率 7%）</h4>
          <table class="table">
            <thead><tr><th>描述</th><th>支出科目</th><th>数量</th><th>单价</th><th>税率</th><th>操作</th></tr></thead>
            <tbody>
...
async function load() {
  ...
  partyById.value = Object.fromEntries(vendors.value.map((x: any) => [x.id, x]))
  ...
  docs.value = await api.listDocuments('PURCHASE_ORDER')
}
...
function openExample() {
  openCreate()
  const v = vendors.value.find((x: any) => String(x.name || '').includes('示例'))?.id || vendors.value[0]?.id || ''
  form.value.vendor_id = v
  form.value.project = '行政部'
  form.value.description = '示例采购：办公用品（含税）'
  form.value.lines = [
    { description: '办公用品', account_id: byCode('5001'), quantity: '1', unit_price: '200', tax_rate: '0.07', memo: '办公用品' },
  ]
}
function close() {
...
async function save() {
  ...
  try {
    ...
    const doc = await api.createPurchaseOrder(payload)
    showDialog.value = false
    await load()
